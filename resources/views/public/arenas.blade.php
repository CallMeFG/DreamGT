        ...
        <div class="max-w-7xl mx-auto px-6 py-20">
            ...
            <div x-show="tab === 'pcs'" x-transition.opacity.duration.500ms>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach($pcs as $pc)
                        <div class="group bg-[#111] border border-white/10 rounded-xl overflow-hidden hover:border-ewc-gold transition-all duration-300 hover:-translate-y-2">
                            <div class="h-48 bg-gray-900 relative overflow-hidden flex items-center justify-center group-hover:bg-gray-800 transition-colors">
                                <svg class="w-24 h-24 text-gray-800 group-hover:text-white/10 transition-colors duration-500" fill="currentColor" viewBox="0 0 24 24"><path d="M4 4h16v12H4z"/><path d="M8 20h8v2H8z"/></svg>

                                <div class="absolute top-4 right-4">
                                    @if($pc->status == 'available')
                                        <span class="bg-green-500/20 text-green-400 border border-green-500/30 text-[10px] font-bold px-3 py-1 rounded uppercase tracking-wider">Ready</span>
                                    @elseif($pc->status == 'booked')
                                        <span class="bg-yellow-500/20 text-yellow-400 border border-yellow-500/30 text-[10px] font-bold px-3 py-1 rounded uppercase tracking-wider">In Use</span>
                                    @else
                                        <span class="bg-red-500/20 text-red-400 border border-red-500/30 text-[10px] font-bold px-3 py-1 rounded uppercase tracking-wider">Maintenance</span>
                                    @endif
                                </div>

                                <div class="absolute bottom-4 left-4">
                                    <h3 class="text-3xl font-black text-white/20 group-hover:text-ewc-gold transition-colors select-none">
                                        {{ $pc->pc_number }}
                                    </h3>
                                </div>
                            </div>
                            ...
                            <div class="p-6">
                                <div class="flex justify-between items-start mb-4">
                                    <div>
                                        ...
                                        <p class="text-gray-500 text-xs mt-1 truncate w-48">{{ $pc->specifications ?? 'High Performance Gaming Setup' }}</p>
                                    </div>
                                    <div class="text-right">
                                        <span class="block text-xl font-bold text-ewc-gold">Rp {{ number_format($pc->type->price_per_hour ?? 0, 0, ',', '.') }}</span>
